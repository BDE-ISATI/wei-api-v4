...
def lambda_handler(event, context):
    try:
        ...
        if 'cognito:groups' in token and 'Admin' in token['cognito:groups']:
            return {
                'statusCode': 401,
                'body': json.dumps({"message": 'You must be a player', "err": "unauthorized"}),
                'headers': {
        ...
        if challenge_id not in user['Item']['challenges_to_do']:
            return {
                'statusCode': 400,
                'body': json.dumps({"message": 'Challenge not in todo', "err": "notInTodo"}),
                'headers': {
                    'Access-Control-Allow-Origin': '*'
                }
            }
        ...
        if response['ResponseMetadata']['HTTPStatusCode'] != 200:
            return {
                'statusCode': 500,
                'body': json.dumps({"message": 'Error removing todo challenge', "err": "dynamodbError"}),
                'headers': {
                    'Access-Control-Allow-Origin': '*'
                }
            }
